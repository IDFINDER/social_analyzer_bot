...
# استخدم نموذج gemini-2.0-flash-lite-001 (أسرع وأكثر استقراراً)
...
async def get_channel_recommendations(channel_details):
    ...
    if not GEMINI_API_KEY:
        return "⚠️ خدمة الذكاء الاصطناعي غير متاحة حالياً (مفتاح API غير موجود)."
    ...
    try:
        ...
        print(f"🔍 [GEMINI] Sending request to: {GEMINI_API_URL}")
        ...
        if response.status_code == 200:
            ...
            print(f"🔍 [GEMINI] Success! Response length: {len(result)}")
            return result
        else:
            ...
            print("🔍 [GEMINI] Trying alternative model...")
            alt_url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-001:generateContent"
            ...
            if response2.status_code == 200:
                ...
                return f"⚠️ عذراً، حدث خطأ في جلب التوصيات (HTTP {response.status_code}). حاول مرة أخرى لاحقاً."
