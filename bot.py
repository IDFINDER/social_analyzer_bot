...
from utils.texts import (
    Buttons, Messages, Errors, AccountMessages, RecommendationMessages,
    DEVELOPER_LINK, BOT_LINK, BOT_NAME, PAYMENT_NUMBER
)
...
def get_main_keyboard(is_premium=False):
    ...
    keyboard = Buttons.MAIN_MENU_FREE if not is_premium else Buttons.MAIN_MENU_PREMIUM
    return ReplyKeyboardMarkup(keyboard, resize_keyboard=True)

def get_analysis_keyboard():
    """لوحة اختيار منصة التحليل"""
    keyboard = []
    for row in Buttons.ANALYSIS_MENU:
        row_buttons = []
        for text in row:
            if "يوتيوب" in text:
                ...
                row_buttons.append(InlineKeyboardButton(text, callback_data="analyze_tiktok"))
            elif "فيسبوك" in text:
                row_buttons.append(InlineKeyboardButton(text, callback_data="analyze_facebook"))
            else:
                row_buttons.append(InlineKeyboardButton(text, callback_data="main_menu"))
...
async def my_data_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not accounts:
        await update.message.reply_text(Errors.NO_ACCOUNTS, reply_markup=get_main_keyboard(False))
        return
    
    user_info = get_user_info(user_id)
    is_premium = user_info['status'] == 'premium' if user_info else False
    status = "👑 مميز" if is_premium else "🎁 مجاني"
...
async def subscription_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, plan_type):
    ...
    plan_details = {
        'monthly': {'name': 'شهري', 'days': 30, 'price': prices.get('price_monthly', 10)},
        'half_yearly': {'name': 'نصف سنوي', 'days': 180, 'price': prices.get('price_half_yearly', 30)},
        'yearly': {'name': 'سنوي', 'days': 365, 'price': prices.get('price_yearly', 48)},
        'lifetime': {'name': 'مدى الحياة', 'days': 36500, 'price': prices.get('price_lifetime', 100)},
        'promo': {'name': 'نصف سنوي (عرض)', 'days': 180, 'price': prices.get('promo_half_yearly', 25)}
    }
    ...
    await query.edit_message_text(
        ...
        f"⚠️ بعد الدفع، تواصل مع المطور {DEVELOPER_LINK} لإرسال إيصال الدفع وتفعيل اشتراكك.",
        parse_mode='HTML',
        reply_markup=keyboard
    )
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = Messages.HELP.format(free_limit=FREE_LIMIT, developer_link=DEVELOPER_LINK)
    
    keyboard = [
        [InlineKeyboardButton("📋 سياسة الخصوصية", url=f"https://{RENDER_URL}/privacy")],
...
async def analyze_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not accounts:
        await update.message.reply_text(Errors.NO_ACCOUNTS, reply_markup=get_main_keyboard(is_premium))
        return
    ...
    if not is_premium and not can_analyze_bool:
        ...
        quota_text = Errors.QUOTA_EXCEEDED.format(
            free_limit=FREE_LIMIT,
            current_uses=current_uses,
            remaining=remaining
        )
        await update.message.reply_text(quota_text, parse_mode='HTML', reply_markup=keyboard)
        return
...
async def analyze_youtube(update: Update, context: ContextTypes.DEFAULT_TYPE, query=None):
    ...
    if not is_premium:
        ...
        if not can_analyze_bool:
            ...
            await message.reply_text(quota_text, parse_mode='HTML', reply_markup=keyboard)
            return
    
    youtube_account = get_user_account(user_id, 'youtube')
    if not youtube_account:
        await message.reply_text(Errors.NO_YOUTUBE, reply_markup=get_main_keyboard(is_premium))
        return
    ...
    if not channel_details:
        await status_msg.edit_text(Errors.CHANNEL_NOT_FOUND)
        return
    ...
    for video in channel_details.get('latest_videos', [])[:10]:
        ...
        if published_at:
            try:
                ...
                posting_times.append({'hour': dt.hour, 'day': dt.strftime('%A')})
            except:
                pass
    ...
    analysis_data = {
        'subscribers': subscribers, 'followers': subscribers, 'total_views': total_views,
        'total_videos': total_videos, 'total_posts': total_videos, 'avg_views_per_post': avg_views,
        'avg_video_duration': 600, 'engagement_rate': engagement_rate,
        'best_posting_hour': best_hour, 'best_posting_day': best_day,
        'top_posts': channel_details.get('latest_videos', [])[:5], 'account_name': account_name
    }
    ...
    message_text, file_data = format_channel_report(channel_details, user_id, is_premium, remaining)
    ...
    if file_data:
        ...
        with open(filename, 'rb') as f:
            await message.reply_document(document=f, filename=filename, caption="📊 ملف التحليل الكامل")
        os.remove(filename)
    else:
        await message.reply_text(message_text, parse_mode='HTML')
    ...
    if is_premium:
        ...
        if growth_metrics.get('subscribers_growth', 0) != 0:
            growth_text = f"📊 <b>التطور منذ آخر تحليل:</b>\n"
            growth_text += f"👥 المشتركين: +{growth_metrics.get('subscribers_growth', 0):,} ({growth_metrics.get('subscribers_percent', 0)}%)\n"
            growth_text += f"👁️ المشاهدات: +{growth_metrics.get('views_growth', 0):,} ({growth_metrics.get('views_percent', 0)}%)\n"
        
        ai_message = Messages.AI_RECOMMENDATIONS_INTRO.format(
            channel_title=channel_details.get('title'),
            growth_text=growth_text,
            rec_length=0,
            remaining=remaining - 1 if remaining else 4
        ) if growth_metrics.get('subscribers_growth', 0) != 0 else "🤖 <b>توصيات الذكاء الاصطناعي</b>\n\n✨ هل تريد الحصول على توصيات مخصصة لتحسين قناتك؟"
        
        keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton("🤖 احصل على توصيات ذكية", callback_data=f"ai_recommendations_{channel_details['channel_id']}")
        ]])
        await message.reply_text(ai_message, parse_mode='HTML', reply_markup=keyboard)

# =================================================================================
async def analyze_snapchat_command(update: Update, context: ContextTypes.DEFAULT_TYPE, query=None):
    """بدء تحليل Snapchat"""
    user_id = update.effective_user.id if not query else query.from_user.id
    ...
    if not token:
        ...
        keyboard = InlineKeyboardMarkup([[
            InlineKeyboardButton("🔐 تفعيل سناب شات", url=auth_url)
        ]])
        await message.reply_text(
            "🔐 <b>تفعيل تحليل سناب شات</b>\n\n"
    ...
    status_msg = await message.reply_text("⏳ جاري تحليل حساب سناب شات...")
    
    report = await format_snapchat_report(user_id)
...
async def ai_recommendations(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_premium:
        await query.edit_message_text(Errors.NOT_PREMIUM)
        return
    ...
    youtube_account = get_user_account(user_id, 'youtube')
    if not youtube_account:
        await query.edit_message_text(Errors.NO_YOUTUBE)
        return
    
    channel_details, _ = await get_channel_details(youtube_account['account_identifier'])
    if not channel_details:
        await query.edit_message_text(Errors.CHANNEL_NOT_FOUND)
        return
    ...
    await query.edit_message_text(RecommendationMessages.PROCESSING)
...
"""
    ...
    if not recommendations or len(recommendations) < 50:
        recommendations = RecommendationMessages.FAILED
    ...
    file_content = RecommendationMessages.FILE_HEADER.format(
        channel_title=channel_details.get('title', 'غير معروف'),
        date=datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        rec_id=saved_rec.get('id', 'N/A') if saved_rec else 'N/A',
        subscribers=channel_details.get('subscribers', '0'),
        views=channel_details.get('total_views', '0'),
        growth=growth_metrics.get('subscribers_growth', 0),
        growth_percent=growth_metrics.get('subscribers_percent', 0),
...
async def bio_page_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_premium:
        await update.message.reply_text(Errors.NOT_PREMIUM)
        return
    
    accounts = get_user_social_accounts(user_id)
    if not accounts:
        await update.message.reply_text(Errors.NO_ACCOUNTS)
        return
    ...
    if existing_bio:
        ...
        avatar_status = '✅ محددة' if avatar and avatar != 'غير محددة' else '❌ غير محددة'
        
        text = Messages.BIO_EXISTS.format(
            full_url=full_url,
            views=views,
            theme_display=theme_display,
            bio_text=bio_text[:200] + ('...' if len(bio_text) > 200 else ''),
        ...
        if page_url:
            full_url = f"https://{flask_url}/bio/{page_url}"
            text = Messages.BIO_NEW.format(full_url=full_url)
            await update.message.reply_text(text, parse_mode='HTML')
        else:
            await update.message.reply_text(Errors.GENERIC_ERROR)
...
async def edit_data_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(AccountMessages.EDIT_DATA_INTRO, parse_mode='HTML', reply_markup=InlineKeyboardMarkup(keyboard))
...
async def handle_edit_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        AccountMessages.SUCCESS_ACCOUNT_UPDATED.format(platform=platform.capitalize(), identifier=escape_html(new_identifier)),
        parse_mode='HTML',
...
async def handle_add_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_premium:
        ...
        for plat, acc in accounts.items():
            formatted_accounts[plat] = {'account_identifier': acc['account_identifier']}
        display_name = user_info.get('first_name', 'مستخدم')
        create_or_update_bio_page(user_id, display_name, formatted_accounts)
    
    await update.message.reply_text(
        AccountMessages.SUCCESS_ACCOUNT_ADDED.format(platform=platform.capitalize(), identifier=escape_html(new_identifier)),
        parse_mode='HTML',
        reply_markup=get_main_keyboard(is_premium)
    )
...
async def edit_display_name_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        AccountMessages.EDIT_DISPLAY_NAME.format(current_name=current_name),
        parse_mode='HTML',
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def handle_display_name_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not new_name:
        await update.message.reply_text(Errors.NAME_EMPTY)
        return
    ...
    if result.data:
        ...
        await update.message.reply_text(
            AccountMessages.SUCCESS_NAME_UPDATED.format(new_name=escape_html(new_name)),
            parse_mode='HTML'
        )
...
async def bio_settings_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bio_page = get_bio_page(user_id)
    if not bio_page:
        await query.edit_message_text(Errors.BIO_NOT_FOUND)
        return
...
async def bio_edit_bio_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["back"], callback_data="bio_settings")]]
...
async def bio_edit_avatar_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["cancel"], callback_data="bio_cancel_edit")]]
...
async def handle_bio_text_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update_bio_text(user_id, new_bio):
        ...
        await update.message.reply_text(Messages.SUCCESS_BIO_UPDATED.format(field="النبذة"))
...
async def handle_avatar_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not avatar_url.startswith(('http://', 'https://')):
        await update.message.reply_text(Errors.INVALID_URL)
        return
    
    if update_bio_avatar(user_id, avatar_url):
        context.user_data.pop('editing_avatar', None)
        await update.message.reply_text(Messages.SUCCESS_BIO_UPDATED.format(field="الصورة الشخصية"))
        await show_bio_management(update, context, user_id)
    else:
        await update.message.reply_text(Errors.GENERIC_ERROR)
...
async def bio_reset_page_warning(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["reset"], callback_data="bio_reset_page")],
        [InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["no"], callback_data="bio_settings")]
    ]
    
    await query.edit_message_text(
        Messages.WARNING_RESET_PAGE,
        parse_mode='HTML',
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def bio_reset_url_warning(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["new_url"], callback_data="bio_reset_url")],
        [InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["no"], callback_data="bio_settings")]
    ]
    
    await query.edit_message_text(
        Messages.WARNING_NEW_URL,
        parse_mode='HTML',
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def bio_delete_page_warning(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["delete_page"], callback_data="bio_delete_page")],
        [InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["no"], callback_data="bio_settings")]
    ]
    
    await query.edit_message_text(
        Messages.WARNING_DELETE_PAGE,
        parse_mode='HTML',
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def bio_reset_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bio_page = get_bio_page(user_id)
    if not bio_page:
        await query.edit_message_text(Errors.BIO_NOT_FOUND, parse_mode='HTML')
        return
    ...
    try:
        ...
        await query.edit_message_text(Messages.SUCCESS_BIO_RESET, parse_mode='HTML')
    except Exception as e:
...
async def bio_reset_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not accounts:
        await query.edit_message_text(Errors.NO_ACCOUNTS, parse_mode='HTML')
        return
...
async def bio_delete_page(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text(Messages.SUCCESS_BIO_DELETED, parse_mode='HTML')
...
async def delete_account_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["yes"], callback_data=f"confirm_delete_{platform}")],
        [InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["no"], callback_data=f"cancel_delete_{platform}")]
    ]
    
    await query.edit_message_text(
        Messages.WARNING_DELETE_ACCOUNT.format(platform=platform.capitalize()),
        parse_mode='HTML',
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def confirm_delete_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        AccountMessages.SUCCESS_ACCOUNT_DELETED.format(platform=platform.capitalize()),
        parse_mode='HTML'
    )
...
async def cancel_delete_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["back"], callback_data=f"edit_{platform}")]]
    
    await query.edit_message_text(
        AccountMessages.EDIT_ACCOUNT.format(platform=platform.capitalize()),
        parse_mode='HTML',
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ========== أزرار تعديل وحذف الحسابات ==========
        ...
        keyboard = [
            [InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["delete_account"], callback_data=f"delete_{platform}")],
            [InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["main_menu"], callback_data="main_menu")]
        ]
        await query.edit_message_text(
            AccountMessages.EDIT_ACCOUNT.format(platform=platform.capitalize()),
    ...
    # ----- أزرار إدارة صفحة البايو (القديمة) -----
        ...
        keyboard = [
            ...
            [InlineKeyboardButton(Buttons.CONFIRM_BUTTONS["back"], callback_data="bio_back")]
        ]
        await query.edit_message_text(
